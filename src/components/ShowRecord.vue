        ...
        <q-list bordered separator>
          <q-infinite-scroll @load="loadMore" :offset="10">
            <task-list-edit
              ...
              :id="key"
            ></task-list-edit>
            <template v-slot:loading v-if="this.actualMaxPosition < this.FindRecordLength">
...
export default {
  ...
  created() {},
  mounted() {
    this.fbReadData(), this.readProfessionalTitle();
  },
  watch: {},
  computed: {
    ...mapState("fieldrecord", ["FieldReord", "tasksDownloaded"]),
    ...mapGetters("fieldrecord", ["FieldReordFiltered", "FindRecordLength"]),
    showingData() {
      let keys = [];
      let showTasks = {};
      let showKeys = [];
